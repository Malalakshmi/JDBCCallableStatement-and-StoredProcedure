package om.csStoredProcedureGetDetailsByName;
...
/*
 CREATE DEFINER=`root`@`localhost` PROCEDURE `Get_Details_By_Name`(IN name1 varchar(50), IN name2 varchar(50))
BEGIN
     select pid,pname,prate,pqnty
     from products
     where pname IN (name1,name2);
END 
 */

public class csStroedProcedureGetDetailsByName {
	private static final String storedProcedureCall = "{CALL Get_Details_By_Name(?,?)}";
